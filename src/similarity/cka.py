...
import src.similarity.experiments
...
def pairwise_cka_full(
    ...
    for seed_pair in itertools.combinations(sorted(dirnames), pair_length):
        ...
        fnames = src.similarity.experiments.find_activation_fnames(
        ...
        with tqdm(total=cka_values.size, desc="Computing CKA values") as t:
            for i, fname1 in enumerate(fnames[0]):
                for j, fname2 in enumerate(fnames[1]):
                    x = src.similarity.experiments.load_representation(
                        os.path.join(os.getcwd(), seed_pair[0], f"{fname1}.pt")
                    )[idx]
                    y = src.similarity.experiments.load_representation(
        ...
        if save_to_disk:
            np.save(
                Path(cka_dir, f"cka_{split_name}_{'_'.join(seed_pair)}.npy"),
                cka_values,
            )
...
def cka_matrix(
    ...
    if mode == "full":
        ckas = pairwise_cka_full(
            dirnames, idx, cka_dir, split_name, activations_root, save_to_disk
        )
    elif mode == "diag":
